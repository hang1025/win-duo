...
(function (root, factory) {
  ...
  const en = {
    tray: {
      ...
      enabled: 'Enabled',
      monitor: 'Watch for a close automatically',
      monitorOn: 'Auto-monitor: on (camera light stays on)',
      monitorOff: 'Auto-monitor: off',
    ...
    params: {
      ...
      cameraDevice: {
        ...
        generic: 'Camera',
        unavailable: 'Saved camera (not connected)',
        hint: 'Which video input to follow the lid with. System default uses the Windows camera; an unavailable selected camera uses the scripted animation.',
      },
      persistentMonitor: {
        label: 'Watch for a close automatically',
        hint: 'Keeps the camera running and starts the fold by itself when the lid closes past the trigger below. The camera light stays on the whole time this is enabled, which is why it is off by default.',
      },
      monitorTriggerAngle: {
        label: 'Auto-trigger travel',
        hint: 'Relative degrees of lid movement below the position where monitoring started. This is not an absolute hinge angle. Smaller values trigger sooner.',
      },
      monitorRearmAngle: {
        label: 'Re-arm travel',
        hint: 'After a run, the lid must come back within this many relative degrees before another automatic trigger. It must stay below the trigger travel.',
      },
      neutralBand: {
        label: 'Flat zone',
  ...
  const zh = {
    ...
    params: {
      ...
      cameraDevice: {
        ...
        default: '系统默认',
        generic: '摄像头',
        unavailable: '已保存的摄像头（未连接）',
        hint: '用哪个视频输入来跟随合盖。系统默认会使用 Windows 选定的摄像头；所选摄像头不可用时会改用脚本动画。',
      },
      persistentMonitor: {
        label: '自动监测合盖',
        hint: '持续占用摄像头，盖子合到设定的触发行程时自动开始折叠。开启期间摄像头指示灯会一直亮着，所以默认关闭。',
      },
      monitorTriggerAngle: {
        label: '自动触发行程',
        hint: '相对监测开始时盖子位置的相对角度，不是绝对的铰链角度。数值越小越早触发。',
      },
      monitorRearmAngle: {
        label: '重新待命行程',
        hint: '一次运行结束后，盖子要回到这个相对角度以内，才会再次自动触发。必须小于触发行程。',
      },
      neutralBand: {
        label: '平整区间',
